...
public class DefaultCommandHandler implements CommandHandler {
    ...
    public void onCommandWrongContext(WrongContextException e) {
        e.getTriggerEvent().getChannel().sendMessage(":no_entry_sign: Command not allowed here !").queue(); //TODO use templates
    }

    @Override
    public void onCommandLowAccessLevel(LowAccessLevelException e) {
        e.getTriggerEvent().getChannel().sendMessage(":no_entry_sign: You don't have the permission to use this command !").queue(); //TODO use templates
    }
    ...
    public void onCommandMissingPermission(MissingPermissionException e) {
        try {
            e.getTriggerEvent().getChannel().sendMessage(f(":no_entry_sign: Sorry i need the permission %s to execute this command !",
                    e.getMissingPermission().getName())).queue(); //TODO use templates
            ...
            e.getTriggerEvent().getAuthor().openPrivateChannel().complete()
                    .sendMessage(f(":no_entry_sign: Sorry, i need the permissions %s and %s to execute the command \"%s\" !",
                            ...
                            e.getCommand().getName())).queue(); //TODO use templates
        }
    }

    @Override
    public void onCommandFailed(CommandFailedException e) {
        e.getTriggerEvent().getChannel().sendMessage(":no_entry_sign: Something wrong happened...").queue(); //TODO use templates
    }

    @Override
    public void onCommandNoPattern(NoPatternException e) {
        e.getTriggerEvent().getChannel().sendMessage(":no_entry_sign: Sorry, the command you just tipped is invalid !").queue(); //TODO use templates
    }

    @Override
    public void onCommandErrorUnknown(CommandException e) {
        e.getTriggerEvent().getChannel().sendMessage(":no_entry_sign: An unknown error happened ! Please contact an administrator !").queue(); //TODO use templates
    }
}
